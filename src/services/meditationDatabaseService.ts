...
export class MeditationDatabaseService {
  static async get(): Promise<MeditationDatabase | null> {
    ...
    const collection =
      db.collection<MeditationDatabaseDocument>(COLLECTION_NAME);
  ...
  static async create(
    data: Omit<MeditationDatabase, "_id" | "createdAt" | "updatedAt">
  ): Promise<MeditationDatabase> {
    const validatedData = meditationDatabaseSchema
  ...
  static async updateGeneral(
    data: UpdateGeneralContent
  ): Promise<MeditationDatabase | null> {
  ...
  static async updateStructure(
    ...
    Object.keys(validatedData).forEach((key) => {
      updateData[`structure.${key}`] =
        validatedData[key as keyof UpdateStructure];
    });
