...
export class ThemeService {
  ...
  static async getActive(): Promise<Theme[]> {
    ...
    const themes = await collection.find({ isActive: { $ne: false } }).toArray();
    return themes.map(theme => ({
      ...theme,
      _id: theme._id?.toString(),
    })) as Theme[];
  ...
  static async updateByCategory(category: string, data: UpdateTheme): Promise<Theme | null> {
    const validatedData = updateThemeSchema.parse(data);
  ...
  static async setActiveByCategory(category: string, isActive: boolean): Promise<Theme | null> {
